...
When anyone sends `/start` the bot greets them by name and shows buttons:
...
**HELP** shows the commands, **ABOUT** shows details of the bot, and
**UPDATE CHANNEL** opens https://t.me/TG_FILE_STORE69. Optionally set `CLONE_URL`
to also show a **CREATE MY OWN CLONE** button.

Admins store messages/files and get shareable links. Anyone opening a link
receives the content (no greeting, just the files).

## Commands

| Command | Who | What it does |
|---|---|---|
| `/start` | everyone | Home message with HELP / ABOUT buttons; opens share links |
...
| `/settings` | admins | Protect content (no forward/save), auto-delete timer, shorten generated links, auto-approve join requests |
...
/forcesub add -1001234567890 request   (REQUEST MODE, see below)
/forcesub list
...
`/forcesub add <channel id> request` makes the bot create a "request to join"
invite link. Users only have to **send a join request** - the bot lets them in
straight away, no need to wait for you to approve. Turn on **Auto-approve
requests** in `/settings` if you also want the bot to approve them for you;
otherwise approve manually in the channel. The bot needs the "Invite users"
admin permission. Note: if the bot's database is wiped (redeploy without a
volume), earlier requests are forgotten - turn auto-approve on to avoid users
getting stuck.
...
| `UPDATE_CHANNEL` | no | Link for the UPDATE CHANNEL button (default https://t.me/TG_FILE_STORE69) |
| `CLONE_URL` | no | If set, shows a CREATE MY OWN CLONE button with this link |
